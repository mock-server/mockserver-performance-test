...
# ./standalone_performance_test_release.sh 2>&1 > standalone_performance_test_release.log
...
do
  java -Xmx1g -Dmockserver.logLevel=WARN \
    ...
    -jar ~/.m2/repository/org/mock-server/mockserver-netty/5.11.0/mockserver-netty-5.11.0-jar-with-dependencies.jar \
    -serverPort 1080 &
  sleep 5
  ...
  do
      locust --loglevel=DEBUG --headless --only-summary --csv="${maxExpectations}_${count}_release" -u $count -r 15 -t 10 --host=http://127.0.0.1:1080 2>&1
